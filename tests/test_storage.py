...
# ---------------------------------------------------------------------------
# Session settings (rescued from session_notes.note_type='settings' in v80)
# ---------------------------------------------------------------------------
...
class TestSessionSettings:
    ...
    async def test_list_settings_keys_empty_when_no_settings(self, storage: Storage) -> None:
    ...
    async def test_list_settings_keys_returns_sorted_distinct_keys(self, storage: Storage) -> None:
        ...
        await storage.create_session_setting(
            session_id=race_id,
            body=_json.dumps({"backstay": "2.5", "cunningham": "off"}),
        )
        await storage.create_session_setting(
            session_id=race_id,
            body=_json.dumps({"jib_lead": "5", "backstay": "3"}),
        )
        keys = await storage.list_settings_keys()
    ...
    async def test_list_settings_keys_ignores_malformed_bodies(self, storage: Storage) -> None:
        race_id = await self._make_race(storage)
        await storage.create_session_setting(session_id=race_id, body="not-json")
        await storage.create_session_setting(session_id=race_id, body="[1, 2, 3]")
        keys = await storage.list_settings_keys()
        assert keys == []
